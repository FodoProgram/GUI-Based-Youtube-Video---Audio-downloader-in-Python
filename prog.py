...
class CustomProgressBar:
    def __init__(self, root):
        ...
        self.remaining_label = tk.Label(self.progress_window, text="Remaining: --:--:--")
    ...
    def update_progress(self, bytes_downloaded, total_size, start_time):
        ...
        if elapsed_time > 0:
            ...
            self.speed_label.config(text=f"Speed: {speed:.2f} B/s")

            remaining_time = (total_size - bytes_downloaded) / speed
            minutes, seconds = divmod(remaining_time, 60)
            hours, minutes = divmod(minutes, 60)
            self.remaining_label.config(text=f"Remaining: {int(hours)}:{int(minutes)}:{int(seconds)}")
...
def download_video():
    ...
    try:
        ...
        youtube = YouTube(url, on_progress_callback=lambda stream, chunk, bytes_remaining: progress_func(stream, chunk, bytes_remaining, progress_bar, start_time))
        streams = youtube.streams.filter(adaptive=True, mime_type='video/mp4')
        stream = None
        ...
        threading.Thread(target=download_thread, args=(stream, video_folder, video_title, stream.filesize, start_time)).start()
...
def download_audio():
    ...
    try:
        ...
        threading.Thread(target=download_thread, args=(audio_stream, audio_folder, audio_title, audio_stream.filesize, start_time)).start()

    except Exception as e:
        messagebox.showerror("Error", str(e))

def download_thread(stream, folder, title, total_size, start_time):
...
def progress_func(stream, chunk, bytes_remaining, progress_bar, start_time):
    total_size = stream.filesize
    bytes_downloaded = total_size - bytes_remaining
    progress_bar.update_progress(bytes_downloaded, total_size, start_time)
